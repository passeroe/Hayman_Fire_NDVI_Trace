...
```{r, echo=FALSE}
knitr::opts_knit$set(root.dir = '../') # tells it to go up a level looking for data
```
...
### Adding water year column
When analyzing water flux data, we typically break the year up into a water year which run from October to the end of September. For this exploratory analysis, we want to group the datasets by water year and then join them to each other so we can compare winter average, max, median, etc. of snow cover versus the next water years water flux. So we have to add a column called water year
...
```{r, echo=FALSE}
# join by water year
combo_SN_Q <- inner_join(q_means,snow_means,by="wtr_yr")

ggplot(combo_SN_Q,aes(x=mean_ndsi,y=median_q,color=wtr_yr)) +
  geom_point()+
...
```{r, echo=FALSE}
ggplot(combo_SN_Q,aes(x=max_ndsi,y=median_q,color=wtr_yr)) +
  geom_point()+
